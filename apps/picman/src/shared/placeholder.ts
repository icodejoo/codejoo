/**
 * Zero-dependency SVG color-block placeholder synthesis, shared by the SW
 * pipeline and the page-side video facade (kept free of any canvas/SW code so
 * page bundles don't pull in Service Worker logic).
 *
 * 零依赖 SVG 色块占位生成,SW 管线与页面端 video facade 共用(不含任何 canvas/SW 代码,
 * 使页面包不会拉入 Service Worker 逻辑)。
 */
...
/**
 * Encode an SVG string as a `data:` URI usable as an `<img>`/`poster` source.
 *
 * 把 SVG 字符串编码为可作 `<img>`/`poster` 源的 `data:` URI。
 * @param svg - SVG markup — SVG 字符串
 * @returns `data:image/svg+xml,...` URI — `data:image/svg+xml,...` URI
 * @example const poster = svgDataUri(svgColorBlock({ width: 16, height: 9, mode: 'gradient', fallbackColor: '#e0e0e0' }))
 */
