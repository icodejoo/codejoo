 ...
 * countDown 插件端到端测试。
 *
 * 独立性验证:仅安装 countDown,不装 tween/animate/countUp,
 * `ticker.countDown` 工作,而其他三个 API 不存在(以证明无隐式依赖)。
 */
...
describe('countDown — 独立性证明', () => {
  it('仅装 countDown,ticker.countDown 可用,to/animate/countUp 不存在', () => {
    const { ticker } = createTestTicker([countDown()])
    expect(typeof (ticker as any).countDown).toBe('function')
...
describe('countDown — Ticker 集成', () => {
  it('每秒回调,到 0 自动停止', () => {
    const { ticker, advance } = createTestTicker([countDown()])
    const cb = vi.fn()
    ticker.countDown(3000, cb)
    advance(1000); advance(2000); advance(3000)
    expect(cb).toHaveBeenCalled()
    const before = cb.mock.calls.length
    advance(4000); advance(5000)
  ...
  it('自定义 interval / formatter', () => {
    ...
    const fmt = buildHighPerfFormatter('mm:ss.sss')
    const calls: string[] = []
    ticker.countDown(2000, txt => calls.push(txt), { interval: 100, formatter: fmt })
    advance(100); advance(200); advance(300)
  ...
  it('remove() 提前取消', () => {
    const { ticker, advance } = createTestTicker([countDown()])
    const cb = vi.fn()
    const ctrl = ticker.countDown(5000, cb)
    advance(1000)
    ctrl.remove()
    advance(2000); advance(3000)
    expect(cb).toHaveBeenCalledTimes(1)
  })

  it('duration 越界检查', () => {
    const { ticker } = createTestTicker([countDown()])
    expect(() => ticker.countDown(1e12, () => {})).toThrow()
    expect(() => ticker.countDown(NaN as any, () => {})).toThrow()
  })
})
