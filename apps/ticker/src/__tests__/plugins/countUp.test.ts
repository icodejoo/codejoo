 ...
 * countUp 插件端到端测试。
 *
 * 独立性证明:仅装 countUp,不装 tween/animate/countDown,
 * `ticker.countUp` 可用,其他三 API 不存在。
 */
import { describe, it, expect, vi } from 'vitest'
import { countUp, buildCountUpFormatter } from '../../index'
import { createTestTicker } from '../helpers'

describe('countUp — 独立性证明', () => {
  it('仅装 countUp,ticker.countUp 可用,to/animate/countDown 不存在', () => {
    const { ticker } = createTestTicker([countUp()])
    expect(typeof (ticker as any).countUp).toBe('function')
    expect((ticker as any).to).toBeUndefined()
...
describe('countUp — Ticker 集成', () => {
  it('callback 形式: 触发回调', () => {
    const { ticker, advance } = createTestTicker([countUp()])
    const cb = vi.fn()
    ticker.countUp(100, cb)
    advance(100); advance(500); advance(1000)
  ...
  it('end 时 progress=1,自动 remove', () => {
    const { ticker, advance } = createTestTicker([countUp()])
    const calls: string[] = []
    ticker.countUp(100, { duration: 500, fps: 0, prefix: '' }, txt => calls.push(txt))
    advance(0); advance(250); advance(500); advance(600)
    const last = calls[calls.length - 1]
    expect(parseFloat(last)).toBeCloseTo(100, 0)
    const beforeLen = calls.length
    advance(700); advance(800)
  ...
  it('el 绑定: 写入 textContent', () => {
    ...
    ticker.countUp(100, { el, duration: 100, fps: 0, prefix: '' })
  ...
  it('既无 el 也无 cb → 抛错', () => {
    const { ticker } = createTestTicker([countUp()])
    expect(() => ticker.countUp(100, {})).toThrow()
  })

  it('update: 平滑重定向', () => {
    const { ticker, advance } = createTestTicker([countUp()])
    const calls: number[] = []
    const ctrl = ticker.countUp(100, { duration: 500, fps: 0, prefix: '' }, txt => calls.push(parseFloat(txt)))
    advance(0); advance(100); advance(200)
    ctrl.update(1000)
    advance(300); advance(700)
    const last = calls[calls.length - 1]
  ...
  it('remove: 取消动画', () => {
    const { ticker, advance } = createTestTicker([countUp()])
    const cb = vi.fn()
    const ctrl = ticker.countUp(100, { duration: 500, fps: 0 }, cb)
    advance(0); advance(100)
    ctrl.remove()
    const before = cb.mock.calls.length
    advance(200); advance(500)
  ...
  it('用户改 prefix 时 formatter 跟随', () => {
    const { ticker, advance } = createTestTicker([countUp()])
    const calls: string[] = []
    ticker.countUp(100, { prefix: '$', duration: 100, fps: 0 }, txt => calls.push(txt))
    advance(0); advance(100)
