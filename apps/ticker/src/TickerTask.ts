 ...
 *
 * 由 {@link TickerManager.add} 内部创建，外部不应直接 new。
 * 所有时间字段单位均为毫秒(ms)。
 *
 ...
 * @example
 * // 通过 Ticker 间接创建（推荐）
 * const id = ticker.setTimeout((task) => {
 *   console.log(task.id, task.updateAt)
 * }, 2000)
 *
 * // 在回调中访问任务状态
 * ticker.setInterval((task) => {
 *   const elapsed = task.updateAt - task.beginAt
 *   console.log(`已运行 ${elapsed}ms`)
 * }, 1000)
 */
export default class TickerTask {
  /** 任务唯一标识，由 TickerManager 的自增计数器分配 */
  declare id: number
  ...
  /**
   * @param id       - 任务唯一 ID，由 TickerManager 提供
