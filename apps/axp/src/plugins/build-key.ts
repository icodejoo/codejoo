 ...
 */
export default function buildKey({ after, before, enable = true, fastMode, ignoreKeys, ignoreValues }: IBuildKeyOptions = {}): Plugin {
    // 插件级默认（请求级未指定时回退到此）
    const defaults: IBuildKeyObject = { fastMode, ignoreKeys, ignoreValues };
    return {
        name: name,
        install(ctx) {
            ctx.request(
