...
import ApiResponse, { ApiError } from '../objects/ApiResponse';
    ...
    install(ctx) {
      ...
      ctx.response(
        function $normalize(response) {
          const api = ApiResponse.fromResponse(response);
        ...
        function $normalize(error: unknown) {
          ...
          const api = ApiResponse.fromResponse(
            err.response ?? { status: err.status ?? 0, data: null },
          );
          (err as AxiosError & { api?: ApiResponse }).api = api;
...
// 工厂名与插件名(kebab)对齐,支持 `api.eject(normalizeResponse)`。
// 函数 .name 只读但可重定义,严格模式 ESM 下必须用 defineProperty 而非直接赋值。
Object.defineProperty(normalizeResponse, 'name', { value: name })
