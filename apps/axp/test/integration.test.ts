...
import { Axp, axpKey as key, axpShare as share, axpCache as cache, axpRetry as retry, axpLoading as loading, axpMock as mock } from '../src';
...
describe('集成 — share「start」高并发去重', () => {
  it('50 个并发同 key 请求 → 真实 HTTP 只发一次，全部拿到同一结果', async () => {
    ...
    use(api, [key(), share({ policy: 'start' })]);
...
describe('集成 — cache 命中（顺序）', () => {
  it('首发未命中→打网络；二发命中→不再打网络', async () => {
    ...
    use(api, [key(), cache({ expires: 10_000 })]);
...
describe('集成 — loading 并发计数', () => {
  it('多并发只 show 一次、hide 一次（计数边界）', async () => {
    ...
    use(api, [loading({ loading: (v) => states.push(v) })]);
