...
import { create, Axp, key, filter, cache, auth, retry, normalize } from '../src';
...
describe('Axp.install — canonical order', () => {
  it('filter runs before key on request: an empty field stripped by filter must not affect the hash key computes', async () => {
    ...
    Axp.install(api, { key: key({ fastMode: false }), filter: filter(), cache: cache() });
  ...
  it('normalize runs last on response: retry gets first crack at a business failure via isExceptionRequest before normalize hard-rejects it', async () => {
    const { net, api } = mkApi();
    let attempts = 0;
    net.fallback(() => {
      attempts++;
    ...
    Axp.install(api, {
      retry: retry({ max: 2, isExceptionRequest: (r) => (r.data as any).code !== 0 }),
      normalize: normalize(),
    });
  ...
  it('omitted plugins are skipped; a single plugin still installs correctly', async () => {
    ...
    Axp.install(api, { key: key() });
    expect(api.plugins().map((p) => p.name)).toEqual(['key']);
  });

  it('the returned AxpHandle looks up installed plugins by name and dispose() ejects exactly that batch', async () => {
    const { net, api } = mkApi();
    net.fallback(() => ({ data: { code: 0, data: { v: 1 } } }));
    const handle = Axp.install(api, { key: key(), cache: cache() });

    expect(handle.plugins.map((p) => p.name)).toEqual(['key', 'cache']);
    expect(handle.plugin('cache')).toBeTruthy();
  ...
  it('append adds after everything the handle tracks', () => {
    ...
    expect(handle.plugins.map((p) => p.name)).toEqual(['key', 'cache', 'custom']);
    expect(api.plugins().map((p) => p.name)).toEqual(['key', 'cache', 'custom']);
  });
  ...
  it('prepend adds before everything the handle tracks', () => {
    ...
    expect(handle.plugins.map((p) => p.name)).toEqual(['custom', 'key', 'cache']);
  });

  it('insertBefore/insertAfter slot a plugin relative to a tracked anchor', () => {
    const { api } = mkApi();
    const handle = Axp.install(api, { key: key(), cache: cache() });
    const keyPlugin = handle.plugin('key')!;
    handle.insertAfter(keyPlugin, namedPlugin('middle'));
    expect(handle.plugins.map((p) => p.name)).toEqual(['key', 'middle', 'cache']);

    handle.insertBefore(keyPlugin, namedPlugin('front'));
    expect(handle.plugins.map((p) => p.name)).toEqual(['front', 'key', 'middle', 'cache']);
  });

  it('insertBefore/insertAfter throw for an anchor this handle does not track', () => {
    const { api } = mkApi();
    const handle = Axp.install(api, { key: key() });
